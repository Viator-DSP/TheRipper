...
namespace viator::gui::components
{
    ...
    public:
        InfoPanel()
        {
            ...
            m_thanks_text_editor.setColour(juce::TextEditor::ColourIds::textColourId,
                                           viator::gui_utils::Colors::text());
            m_thanks_text_editor.setColour(juce::TextEditor::ColourIds::highlightedTextColourId,
                                           juce::Colours::transparentBlack);
            m_thanks_text_editor.setColour(juce::TextEditor::ColourIds::highlightColourId,
                                           viator::gui_utils::Colors::text());
            m_thanks_text_editor.setColour(juce::TextEditor::ColourIds::backgroundColourId,
                                           juce::Colours::transparentBlack);
            m_thanks_text_editor.setColour(juce::TextEditor::ColourIds::outlineColourId,
                                           juce::Colours::transparentBlack);
            ...
            m_manual_button->onClick = [this]()
            {
                const auto tempPdf =
                        ...
                        .getChildFile("Ripper_User_Manual.pdf");

                const bool ok = tempPdf.replaceWithData(
                    BinaryData::Ripper_User_Manual_pdf,
                    BinaryData::Ripper_User_Manual_pdfSize);

                jassert(ok);

                if (ok)
                    tempPdf.startAsProcess();
            };
            ...
            m_license_button->onClick = [this]()
            {
                const auto tempPdf =
                        juce::File::getSpecialLocation(juce::File::tempDirectory)
                        .getChildFile("EULA.pdf");

                const bool ok = tempPdf.replaceWithData(
                    BinaryData::EULA_pdf,
                    BinaryData::EULA_pdfSize);
        ...
        void paint(juce::Graphics &g) override
        {
            ...
            g.drawFittedText(juce::String(JUCE_PROJECT_NAME) + " " + juce::String(JUCE_PROJECT_VERSION),
                             title_bounds,
                             juce::Justification::centred, 2);

            // THANKS TEXT
            g.setFont(viator::gui_utils::Fonts::regular(
                static_cast<float>(getHeight()) * m_paragraph_text_size));
            g.drawFittedText(
        ...
        void resized() override
        {
            ...
            m_thanks_text_editor.setBounds(0, juce::roundToInt(getHeight() * 0.88), getWidth(),
                                           getHeight() / 10);
            m_thanks_text_editor.applyFontToAllText(
                viator::gui_utils::Fonts::regular(static_cast<float>(getHeight()) * m_paragraph_text_size));
        }
    ...
    private:
        ...
        {
            button = std::make_unique<juce::DrawableButton>(
                name, juce::DrawableButton::ButtonStyle::ImageStretched);
